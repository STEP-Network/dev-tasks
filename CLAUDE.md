...
**Bug agent attribution:** both `createBug` and `updateBug` accept an optional `filedByAgent` (Agent-ID enum: Claude Code CLI / Claude Desktop Cloud / Codex Local / Claude Desktop Local / Codex Cloud), written label-based to the Bugs "Filed By Agent" dropdown with `create_labels_if_missing`. Omit for human-filed bugs. The "Source Tool" column (`color_mm3bqre`: sentry/checkly/posthog/vercel/…) is owned by the observability webhook bridge — the MCP tools never write it.
