...
To activate the opt-in hooks (worktree-required, worktree-path-boundary and the rest of `hooks.enabled[]`, plus task-state-guard on the Monday pipeline only), copy `plugin/templates/starter-project-config.json` to `<consumer-project>/.claude/project-config.json` and trim it to what you want enabled. Without that file every opt-in hook is dormant, `rule-autoload` included (opt-in since 1.0.1: plugin rules are read on demand). The always-on hooks still run: the policy hooks `bash-guard`, `stop-ci-green-check` and `people-doors-guard` (no agent session writes as a person through the Monday, Slack, Linear or dev-tasks tools or APIs; its list is root's, `/etc/dev-tasks/people-doors.json`, STEP-3330), the worktree janitor, and `active-task-recon`.
...
**The 1.0 flow:** `/dev` → work → `/ship`, with `/preview` to show a branch on its Vercel preview. `/dev` cuts the branch from the issue id and loads the issue as context without writing to the tracker. `/ship` typechecks, pushes, makes sure the branch has an issue, opens the PR, arms auto-merge where the base allows it, and stops: CI owns everything after the PR opens, and review is the `Claude review` check. A machine profile (`~/.claude/dev-tasks-profile.json`, `human` or `agent`) decides which hooks run, and `tracker.provider` in `.claude/project-config.json` picks Linear (team STEP) or Monday (the default) behind `plugin/scripts/trackerctl.ts`. Install and upgrade steps: [`docs/dev-tasks-1-0-install.md`](docs/dev-tasks-1-0-install.md). Retired in 1.0: `bash-guard` gate (b) and the `stop-task-check`, `post-self-review`, `subtask-reminder` and `subtask-progress-gate` hooks. Off on both profiles: `pipeline-reminder` and `stop-visual-diff-check`.

**The Monday pipeline** still ships for consumers that have not moved over, and most of this file describes it. Canonical walkthrough: [`plugin/rules/workflow-pipeline.md`](plugin/rules/workflow-pipeline.md). End-to-end lifecycle (pickup → ship → release), per-phase hook table, agent vs human role split, and the v0.16.0 push-guard + active-task-integrity enforcement pattern. Read that first; the per-phase rules go deeper on each step.

Plugin skills wrap most of that pipeline: `/dev-tasks:pickup-task`, `create-task`, `refine-task`, `log-progress`, `self-review`, `ship-pr`, `release-version`, `write-uat-spec`.
...
**The four most-skipped steps** — each got a workflow-enforcement hook (opt-in via `project-config.json` → `hooks.enabled[]`), and all but the second still has one:
...
2. **Subtask board state** — no longer hook-enforced: 1.0 retired `subtask-progress-gate`, which refused a push when subtasks existed but none was Done with actualHours. `/log-progress` still advances the board **state** (status + hours), not a narrative Update — see "Monday Updates policy" below.
...
1. **Board state** — status transitions + subtask → Done with `actualHours`. Keeps the board accurate at a glance. (`/log-progress SUBTASK_COMPLETED` advances this state without posting an Update. The `subtask-progress-gate` that enforced the hours was retired in 1.0.)
...
- **`protect-active-task-state`** *(v0.16.0)* — refuses `Edit/Write/MultiEdit` on `.claude/active-task.json` that mutates protected fields (`reviewAddressed`, `parentStatus`, `mondayReconciledShas`, `allowMainCheckout`, `ciGate` since v0.26.0, `stagingDeployReady` since v0.32.0) without a corresponding skill-emission marker in `/tmp`. Closes the 2026-05-27 polads-style bypass where an agent wrote the state file directly. `allowMainCheckout: true` is always blocked (no marker path). `selfReviewPassed` is no longer protected: 1.0 retired `post-self-review`, the only emitter of its marker, so `/self-review` writes the field directly. See `plugin/rules/agent-orchestration.md` "Protected state fields" for the full marker contract and the GitHub branch protection complement.
...
- **Retired in 1.0:** `stop-task-check` (its Stage 3 CI relaxation went with it), `subtask-progress-gate`, `post-self-review`, `subtask-reminder`, and `bash-guard` gate (b). The schema still accepts the hook names in `hooks.enabled[]` so existing configs stay valid, and the runtime ignores them. `/dev-tasks:doctor` Check 16 warns when one is still listed.
...
The hook silent-no-ops when: cwd is not under `.claude/worktrees/`, the tracker provider is `linear` (since 1.0.1, because every case is Monday state: `DEV_TASKS_TRACKER`, else `tracker.provider` in the config at the git toplevel, read by `tracker_provider` in `hooks/lib/config-reader.sh` the same way trackerctl reads it), `MONDAY_API_KEY` is unset, `curl`/`jq` missing, or the Monday API returns no response. One non-silent edge case: if `active-task.json` is present but `taskId` is empty/malformed, the hook emits a one-line warning ("active-task.json present but taskId empty/malformed; skipping drift check.") before exiting 0 — surfacing the broken state file is more useful than silently skipping it. Output format: `[active-task-recon] Case X: <one-line summary>` followed by indented suggestion lines. Smoke-tested by `plugin/hooks/__tests__/active-task-recon.test.sh` (14 cases). Tests 1-12 make no network call: they cover the provider gate, a session started in a subdirectory, and the Monday round-trip against a stubbed `curl`. Tests 13-14 repeat the round-trip against the live Tasks board, creating and deleting an item, so they run only with `DEV_TASKS_LIVE_MONDAY_TESTS=1` as well as `MONDAY_API_KEY` (agent shells export the key, so it is not enough on its own).
