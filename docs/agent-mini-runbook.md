...
`worker.fanOut` lets a develop or revise worker split its work over
subagents (the Agent tool), in the background or the foreground, each on the
model it picks (fable, opus, sonnet or haiku). Its brief says so. Off by
default: no subagents, as before. The web, skills and the Workflow tool stay
off either way (a workflow runs only on a person's own request, and a
worker's brief is not one), and a worker has no agent teams. A subagent runs
in the worker's own process, and `runtime/src/__tests__/sessions.test.ts`
proves on the binary workers run that it is held as the worker is, with no
prompt asked. The sandbox refuses a write outside the worktree and a read of
`~/.config` that no guard recognises. The worker's guard refuses `~/.config`
and `gh pr create`. The plugin's guard refuses `git reset --hard` and `git
push origin main`. Its turns and spend count toward the worker's limits. The
template turns it on, as it is on Eve.
