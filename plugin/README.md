...
| No push to a protected branch | `bash-guard.sh` gate (f) | Any `git push` whose refspecs, remote word or implicit current branch name one of `git.protectedBranches[]` (default `main staging master production prod`, also with no config file), `--all`/`--mirror`/`--branches`, and a push behind a global option (`git -C dir push`). Fails closed (v1.3.8, STEP-3354): input it cannot read, no `python3` or `jq`, a config that is there but unreadable, a command it cannot parse, a git error, or a check that ends without a verdict blocks the command, in every gate. |
