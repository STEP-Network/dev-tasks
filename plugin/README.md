...
- **Agent claims (1.1.0, Linear only)** — every person and every agent has their own Linear account, so a claim is the assignment to the key's own user plus a `claimed by <mini> at <time>` comment whose edit time is the heartbeat. `trackerctl claim` and `heartbeat` act as the profile's `mini` and refuse on a machine without one. `whoami`, `update`, `release`, `claims` and `list --state` are the rest of what the agent runtime uses.
