...
| No agent session writes as a person (v1.3.3, STEP-3330) | `people-doors-guard.sh` | A claude.ai Monday item write or update on a people's board, a claude.ai Slack message in a people's channel or to an agent's bot, an answer entry or a recorder's plan label through the Linear connector, and any change to its own list (`~/.config/dev-tasks/people-doors.json`) other than `people-doors-guard.mjs add`. No list: every Monday item write and Slack send is refused. See docs/agent-mini-runbook.md, The Monday board. |
