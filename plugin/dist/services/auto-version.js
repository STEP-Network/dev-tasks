...
import { BOARDS, TASK_COLUMNS, VERSION_COLUMNS, VERSION_STATUS, VERSION_GROUPS, } from "../constants.js";
import { refreshChangelogForVersion } from "./changelog-refresh.js";
import { getProductIdByName } from "./products.js";
import { getColumnText, getLinkedItems, getMirrorDisplayValue, buildColumnValues, } from "../tools/utils.js";
import { computeBumpSuggestion, parseSemVer, formatSemVer, compareSemVer, } from "./version-bump.js";
const COLD_START = { major: 0, minor: 0, patch: 0 };
/**
 * Main entry. Returns:
 ...
 */
export async function autoAssignVersionForTask(taskId) {
 ...
 */
async function refreshChangelogAndDescribe(versionId) {
    try {
        const r = await refreshChangelogForVersion(versionId);
        const parts = [`${r.taskCount} public task${r.taskCount === 1 ? "" : "s"}`];
        if (r.bugCount > 0)
            parts.push(`${r.bugCount} bug${r.bugCount === 1 ? "" : "s"}`);
        if (r.skippedPrivate > 0)
            parts.push(`${r.skippedPrivate} private skipped`);
...
// Internal helpers
// ---------------------------------------------------------------------------
async function findLatestReleasedForProduct(productId) {
  ...
  `;
    ...
    for (const item of items) {
        ...
        if (!semver)
            continue;
        if (compareSemVer(semver, highest) > 0)
            highest = semver;
    }
    return highest;
}
 ...
 */
async function findBestOpenVersionForProduct(productId) {
...
async function createVersionItem(name, versionNumber, productId, status) {
  ...
  `;
    const res = await executeMondayQuery(mutation);
    const created = res.create_item;
    if (!created)
        throw new Error(`Failed to create version "${name}"`);
    return { id: Number(created.id), name: created.name };
}
async function linkTaskToVersion(taskId, versionId) {
    const columnValues = {
