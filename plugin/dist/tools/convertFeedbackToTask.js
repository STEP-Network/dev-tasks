...
export async function convertFeedbackToTask(args) {
    ...
    `;
        const feedbackResponse = await executeMondayQuery(feedbackQuery);
        const feedbackItem = feedbackResponse.items?.[0];
        if (!feedbackItem) {
            return formatError(`Feedback item #${feedbackId} not found.`);
        }
        const colMap = new Map(feedbackItem.column_values?.map((c) => [c.id, c]) || []);
        // Extract fields
    ...
    `;
        await executeMondayQuery(statusQuery);
        // Format output
        const lines = [];
