...
import { categoryForTaskType, emptyChangelog, writeChangelog } from "./structuredChangelog.js";
export async function generateChangelog(args) {
    ...
    `;
        ...
        if (fixCount > 0)
            summaryParts.push(`${fixCount} fix${fixCount > 1 ? "es" : ""}`);
        ...
        if (breakingChanges?.length)
            structured.breakingChanges = breakingChanges;
        if (knownIssues?.length)
            structured.knownIssues = knownIssues;
        ...
        const sectionTitle = { Feature: "New Features", Fix: "Fixes", Improvement: "Improvements" };
        const sectionOrder = ["Feature", "Fix", "Improvement"];
        const outputLines = [
            `# Changelog Generated`,
            ``,
        ...
        if (highlights?.length)
            outputLines.push(`- **Highlights:** ${highlights.length}`);
        if (breakingChanges?.length)
            outputLines.push(`- **Breaking Changes:** ${breakingChanges.length}`);
        if (knownIssues?.length)
            outputLines.push(`- **Known Issues:** ${knownIssues.length}`);
        if (skippedPrivate > 0) {
            ...
            if (skipReasons.length > 5)
                outputLines.push(`  - …and ${skipReasons.length - 5} more`);
        }
