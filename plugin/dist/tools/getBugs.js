...
import { getColumnText, getLinkedItems, formatError } from "./utils.js";
export async function getBugs(args) {
    try {
    ...
    `;
        ...
        if (productId)
            filterParts.push(`product: #${productId}`);
        if (epicId)
            filterParts.push(`epic: #${epicId}`);
        if (search)
