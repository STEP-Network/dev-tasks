...
import { VERSION_COLUMNS, TASK_COLUMNS, EPIC_COLUMNS, BUG_COLUMNS } from "../constants.js";
import { getColumnText, getColumnValue, getLinkedItems, parseMondayDate, resolveLinkedItems, formatError } from "./utils.js";
export async function getVersion(args) {
    ...
    `;
        ...
        if (docValue) {
          ...
          `;
                    const docResponse = await executeMondayQuery(docQuery);
                            ...
                            try {
                                ...
                                if (parsed.deltaFormat) {
                                    const text = parsed.deltaFormat
                                        ...
                                        .join("");
                                    if (text.trim())
                                        blockTexts.push(text.trim());
                                }
                                else if (typeof parsed === "string") {
                                    if (parsed.trim())
                                        blockTexts.push(parsed.trim());
                                }
                            }
                            catch {
                                // Raw string content
                                if (block.content.trim())
                                    blockTexts.push(block.content.trim());
                            }
                        }
                    }
        ...
        if (expectedDate)
            lines.push(`- **Expected Release:** ${expectedDate}`);
        if (releaseDate)
            lines.push(`- **Release Date:** ${releaseDate}`);
        lines.push("");
        // Product
        ...
        if (resolvedEpics.length > 0) {
            ...
            for (const epic of resolvedEpics) {
                const epicColMap = new Map(epic.column_values?.map((c) => [c.id, c]) || []);
