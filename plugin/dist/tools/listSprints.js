...
import { getColumnText, getLinkedItems, getMirrorDisplayValue, mondayItemUrl, parseMondayDate, resolveLinkedItems, todayDate, formatError } from "./utils.js";
...
export async function listSprints(args) {
    try {
        const { activeOnly = false, pastOnly = false, includeStatusCounts = true, includeTasks = false, format = "markdown", } = args;
    ...
    `;
        ...
        items.sort((a, b) => {
            const aMap = new Map(a.column_values?.map((c) => [c.id, c]) || []);
            const bMap = new Map(b.column_values?.map((c) => [c.id, c]) || []);
            if (pastOnly) {
        ...
        const taskEpicsMap = new Map();
        const taskMetaMap = new Map();
        if (allTaskIds.size > 0) {
            const cols = [TASK_COLUMNS.epic, TASK_COLUMNS.status];
        ...
        const sprints = items.map((item) => {
            ...
            for (const t of linkedTasks) {
                ...
                for (const e of epics)
                    epicsById.set(Number(e.id), e.name);
            }
            const entry = {
                id: Number(item.id),
                name: item.name,
            ...
            if (includeStatusCounts && linkedTasks.length > 0) {
                const counts = Object.fromEntries(STATUS_COUNT_ORDER.map(s => [s.label, 0]));
                ...
                for (const t of linkedTasks) {
                    ...
                    if (!meta)
                        continue;
                    if (counts[meta.status] !== undefined)
                        counts[meta.status]++;
