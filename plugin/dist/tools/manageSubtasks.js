...
function resolveSubtaskId(subtaskId, subtaskName, existingSubtasks, actionLabel) {
    if (subtaskId)
        return subtaskId;
    if (subtaskName) {
        const match = existingSubtasks.find(s => s.name.toLowerCase() === subtaskName.toLowerCase());
        if (match)
            return match.id;
        // Try partial match
        const partialMatch = existingSubtasks.find(s => s.name.toLowerCase().includes(subtaskName.toLowerCase()));
        if (partialMatch)
            return partialMatch.id;
        throw new Error(`Cannot ${actionLabel}: subtask named "${subtaskName}" not found. ` +
...
export async function manageSubtasks(args) {
    try {
        const { parentItemId, operations } = args;
        // Determine if we need to fetch existing subtasks for name-based lookups
        const needsLookup = operations.some(op => (op.action === "update" || op.action === "delete") && !op.subtaskId && op.subtaskName);
        ...
        for (const op of operations) {
            ...
            `;
                        const createResponse = await executeMondayQuery(createMutation);
                    ...
                    // =================================================================
                    case "update": {
                        const subtaskId = resolveSubtaskId(op.subtaskId, op.subtaskName, existingSubtasks, "update");
                        const columnValues = {};
                        const updateDetails = [];
                        if (op.status !== undefined) {
                            columnValues[SUBTASK_COLUMNS.status] = { index: SUBTASK_STATUS[op.status] };
              ...
              `;
                            await executeMondayQuery(updateMutation);
                        }
              ...
              `;
                    ...
                    // =================================================================
                    case "delete": {
                        const subtaskId = resolveSubtaskId(op.subtaskId, op.subtaskName, existingSubtasks, "delete");
