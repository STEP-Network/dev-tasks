...
import { evaluatePublicVisibility, getColumnText, formatError } from "./utils.js";
import { readStructuredFromVersionDoc, writeUnifiedChangelogToVersion, } from "../services/changelog-doc.js";
...
export function serializeForStorage(c) {
    const out = { version: c.version };
    if (c.summary !== undefined)
        out.summary = c.summary;
    if (c.highlights !== undefined)
        out.highlights = c.highlights;
    if (c.breakingChanges !== undefined)
        out.breakingChanges = c.breakingChanges;
    if (c.knownIssues !== undefined)
        out.knownIssues = c.knownIssues;
    out.tasks = {
...
// =============================================================================
export function parseRawChangelog(raw) {
    if (!raw || !raw.trim())
        return undefined;
...
function lookupCategory(bucket) {
    return LEGACY_CATEGORY_MAP[bucket.toLowerCase()];
}
export function migrateChangelog(parsed) {
    ...
    if (Array.isArray(obj.highlights))
        out.highlights = obj.highlights.filter((h) => typeof h === "string");
    if (Array.isArray(obj.breakingChanges))
        out.breakingChanges = obj.breakingChanges.filter((h) => typeof h === "string");
    if (Array.isArray(obj.knownIssues))
        out.knownIssues = obj.knownIssues.filter((h) => typeof h === "string");
    ...
    const taskBuckets = (obj.tasks && typeof obj.tasks === "object") ? obj.tasks
        : (obj.categories && typeof obj.categories === "object") ? obj.categories
            : obj;
    for (const [bucket, value] of Object.entries(taskBuckets)) {
        ...
        for (const entry of value) {
            ...
            if (normalized)
                out.tasks[target].push(normalized);
        }
    }
    return out;
}
function normalizeEntry(raw) {
    ...
    if (raw && typeof raw === "object") {
        ...
        if (typeof o.id === "number")
            ...
            entry.id = Number(o.id);
        if (typeof o.publicName === "string")
            entry.publicName = o.publicName;
        return entry;
    }
    return undefined;
}
 ...
 */
async function readChangelog(versionId) {
  ...
  `;
    ...
    if (text.trim()) {
        ...
        try {
            ...
            if (maybeWrapper && typeof maybeWrapper === "object" && !Array.isArray(maybeWrapper) && typeof maybeWrapper.text === "string") {
                raw = maybeWrapper.text;
            }
            else {
                raw = value;
            }
 ...
 */
export async function writeChangelog(versionId, data, opts) {
    ...
    `;
        await executeMondayQuery(clearMutation);
    }
    catch {
...
async function fetchTaskForChangelog(taskId) {
    const query = `
    query {
      items(ids: [${taskId}]) {
        id
...
// =============================================================================
export async function getStructuredChangelog(args) {
    try {
...
// =============================================================================
export async function updateStructuredChangelog(args) {
    try {
        ...
        if (raw.trim().length > 0 && isEmptyChangelog(current)) {
            return formatError(`Refusing to update version #${versionId}: storage has content but the parser returned empty buckets. ` +
        ...
        for (const op of patch) {
            switch (op.op) {
                case "addTask": {
                    if (op.taskId) {
                        ...
                        if (!fetched.isPublic || !fetched.publicName) {
                            return formatError(`Task #${op.taskId} is private and cannot be added to the changelog (${fetched.visibilityReasons.join(", ")}). ` +
                ...
                case "removeTask": {
                    let removed = 0;
                    if (op.taskId) {
                        for (const cat of ["Feature", "Fix", "Improvement"]) {
                        ...
                        current.tasks[op.category] = current.tasks[op.category].filter(e => !(e.name === op.name && e.id === undefined));
...
// =============================================================================
export async function migrateStructuredChangelog(args) {
    try {
