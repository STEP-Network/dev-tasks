...
import { downloadMondayAsset, fetchTaskAssets, resolveDownloadDir, } from "../monday-client.js";
import { buildSafeAssetFileName, selectAssetDownloadUrl, } from "../monday-assets.js";
...
export async function listTaskAttachments(args) {
    try {
        const { itemId } = args;
        ...
        lines.push(`${assets.length} attachment(s)${includeUpdates ? " (item + updates)" : " (item file columns only)"}:`);
        lines.push("");
        for (const asset of assets) {
            lines.push(describeAsset(asset));
        }
        lines.push("");
        lines.push("Download with `downloadTaskAttachments` (pass `assetIds` to select specific ones), then Read the returned local paths.");
        return lines.join("\n").trim();
    }
    catch (error) {
        return formatError(`Failed to list attachments: ${error instanceof Error ? error.message : String(error)}`);
    }
}
export async function downloadTaskAttachments(args) {
    try {
        const { itemId } = args;
        const includeUpdates = args.includeUpdates ?? true;
        const { found, itemName, assets, warning } = await fetchTaskAssets(itemId, { includeUpdates });
        ...
        if (selected.length === 0) {
            ...
            lines.push(args.assetIds && args.assetIds.length > 0
        ...
        const maxBytes = args.maxFileSizeMb != null ? Math.round(args.maxFileSizeMb * 1024 * 1024) : undefined;
        ...
        return formatError(`Failed to download attachments: ${error instanceof Error ? error.message : String(error)}`);
    }
}
