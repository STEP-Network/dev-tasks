...
import { uploadFileToMonday } from "../monday-client.js";
import { appendDocContent, appendImageBlock, ensureItemDoc, fetchItemName, } from "../services/doc-utils.js";
...
export async function appendTaskVisualSnapshots(args) {
    try {
        ...
        for (const cap of captures) {
            ...
            if (cap.imagePath) {
                try {
                    const fileName = `task-${taskId}-${phase}-${slugify(label)}${cap.viewport ? `-${cap.viewport}` : ""}.png`;
                    const { assetId } = await uploadFileToMonday(taskId, TASK_COLUMNS.attachments, cap.imagePath, fileName);
        ...
        if (failures.length > 0) {
            lines.push(``, `**Upload failures (${failures.length}):**`, ...failures.map((f) => `- ${f}`));
        }
        return lines.join("\n");
    }
    catch (error) {
        return formatError(`Failed to append visual snapshots: ${error instanceof Error ? error.message : String(error)}`);
    }
}
