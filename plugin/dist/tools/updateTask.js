...
import { buildColumnValues, formatError, formatSubtask, getLinkedItems, validateReadyToStart, validateTaskInActiveSprint, validateWaitingForUAT, } from "./utils.js";
export async function updateTask(args) {
    try {
      ...
      `;
        ...
        if (args.status !== undefined) {
            ...
            if (args.status === "Ready to Start") {
                ...
                if (!check.valid) {
                    ...
                    return formatError(`Cannot set task #${itemId} to "Ready to Start". The task is not fully specified:\n${list}`);
                }
            }
            ...
            if (args.status === "Waiting for UAT") {
                ...
                if (check.blockers.length > 0) {
                    const list = check.blockers.map(b => `  - ${b}`).join("\n");
                    return formatError(`Cannot set task #${itemId} to "Waiting for UAT":\n${list}`);
                }
                for (const w of check.warnings)
                    warnings.push(`Waiting for UAT: ${w}`);
            }
          ...
          `;
                    const sprintResponse = await executeMondayQuery(sprintQuery);
                    const sprintCols = sprintResponse.items?.[0]?.column_values || [];
                    const colMap = new Map(sprintCols.map((c) => [c.id, c]));
                ...
                if (!sprintCheck.valid) {
                    return formatError(`Cannot set task #${itemId} to "In Progress".\n${sprintCheck.message}`);
                }
            }
        ...
        `;
                const subtaskResponse = await executeMondayQuery(subtaskQuery);
                const subitems = subtaskResponse.items?.[0]?.subitems || [];
                if (subitems.length > 0) {
                    const incomplete = subitems.filter((sub) => {
                    ...
                    if (incomplete.length > 0) {
                        const incompleteList = incomplete
                            ...
                            .join("\n");
                        return formatError(`Cannot set task #${itemId} to "Done".\n` +
        ...
        if (args.branch !== undefined) {
            try {
                ...
                const branchColumnId = TASK_COLUMNS["branch"];
                if (branchColumnId) {
        ...
        if (args.acceptanceCriteria !== undefined) {
            try {
                const acColumnId = TASK_COLUMNS["acceptanceCriteria"];
                if (acColumnId) {
        ...
        if (args.status !== undefined) {
            ...
            if (args.status === "Waiting for UAT") {
                try {
                    ...
                    warnings.push(`Auto-version assignment failed: ${e instanceof Error ? e.message : String(e)}`);
                }
            }
