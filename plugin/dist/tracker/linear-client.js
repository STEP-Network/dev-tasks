 ...
 */
export function linearEndpoint(env = process.env) {
...
export function resetLinearClientForTests() {
    lastCall = 0;
    writesPerformed = 0;
}
export function loadLinearKey() {
    ...
    try {
        ...
        throw new Error(`LINEAR_API_KEY is not set and ${path} could not be read. ` +
    ...
    if (!key)
        throw new Error(`LINEAR_API_KEY is empty in ${path}`);
    return key;
}
...
export async function linearRequest(query, variables = {}) {
    ...
    if (isMutation) {
        ...
        if (writesPerformed >= cap) {
            throw new Error(`Refusing the write: TRACKERCTL_MAX_WRITES=${cap} reached. ` +
