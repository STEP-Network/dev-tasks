...
import { linearRequest } from "./linear-client.js";
import { byPriorityThenAge, CLAIM_PREFIX, claimCommentBody, extractAcceptanceCriteria, LINEAR_TEAM_KEY, newestClaim, withHeartbeat, } from "./types.js";
const ISSUE_FIELDS = `
  id
  identifier
  title
 ...
 */
async function allNodes(page, next) {
 ...
 */
async function readBackAfterFailure(error, readBack) {
...
export function createLinearTracker() {
    ...
    async function team() {
        ...
        const data = await linearRequest(`query($key: String!) {
        ...
        if (!found) {
            throw new Error(`Linear: no team with key ${LINEAR_TEAM_KEY}. ` +
        ...
        const labels = await allNodes(found.labels, async (after) => {
            const next = await linearRequest(`query($id: String!, $after: String!) {
           team(id: $id) {
    ...
    async function viewer() {
        if (viewerCache)
            return viewerCache;
        const data = await linearRequest(`query { viewer { id name email } }`);
     ...
     */
    async function strictLabelIds(names) {
        const t = await team();
        return names.map((name) => {
            const hit = t.labels.find((l) => l.name === name);
            if (!hit)
                throw new Error(`Linear: no label named ${name}`);
            return hit.id;
        });
    }
    async function fetchRaw(ref) {
        ...
        if (UUID_RE.test(trimmed)) {
            const data = await linearRequest(`query($id: String!) { issue(id: $id) { ${ISSUE_FIELDS} } }`, { id: trimmed });
            if (!data.issue)
                throw new Error(`Linear: no issue with id ${trimmed}`);
            return data.issue;
        }
        ...
        const data = await linearRequest(`query($teamKey: String!, $number: Float!) {
        ...
        if (!found)
            throw new Error(`Linear: no issue ${trimmed} on team ${m[1].toUpperCase()}`);
        return found;
    }
    /**
     ...
     */
    async function listInState(stateName, limit) {
        const where = { teamKey: LINEAR_TEAM_KEY, stateName };
        const page = async (after) => {
            const data = await linearRequest(`query($teamKey: String!, $stateName: String!, $first: Int!, $after: String) {
           issues(
             first: $first,
             after: $after,
        ...
        for (let i = 0; i < ranked.length; i += FULL_ISSUE_CHUNK) {
            ...
            const data = await linearRequest(`query($teamKey: String!, $stateName: String!, $ids: [ID!]!, $first: Int!) {
           issues(
             first: $first,
             filter: {
               id: { in: $ids },
    ...
    async function createComment(issueId, body) {
        ...
        const id = randomUUID();
        try {
            await linearRequest(`mutation($input: CommentCreateInput!) { commentCreate(input: $input) { success } }`, { input: { id, issueId, body } });
        }
        catch (error) {
            await readBackAfterFailure(error, async () => {
                const data = await linearRequest(`query($id: String!) { comment(id: $id) { id } }`, { id });
                return data.comment;
            });
        }
    }
    const tracker = {
        ...
        async createIssue(input) {
            ...
            const payload = { id, teamId: t.id, title: input.title };
            ...
            let created;
            try {
                const data = await linearRequest(`mutation($input: IssueCreateInput!) {
            ...
            if (created.id !== id) {
                ...
                process.stderr.write(`dev-tasks: Linear gave ${created.identifier} its own id, not the client id sent; ` +
        ...
        async attachLink(ref, url, title) {
            const raw = await fetchRaw(ref);
            await linearRequest(`mutation($issueId: String!, $url: String!, $title: String) {
        ...
        async updateIssue(ref, patch) {
            ...
            if (patch.state !== undefined) {
                ...
                if (!stateId)
                    throw new Error(`Linear: no state named ${patch.state} on team ${LINEAR_TEAM_KEY}`);
                input.stateId = stateId;
            }
            if (patch.addLabels?.length)
                input.addedLabelIds = await strictLabelIds(patch.addLabels);
            if (patch.removeLabels?.length)
                input.removedLabelIds = await strictLabelIds(patch.removeLabels);
            if (patch.assignee === "me")
                input.assigneeId = (await viewer()).id;
            if (patch.assignee === null)
                input.assigneeId = null;
            const raw = await fetchRaw(ref);
            if (Object.keys(input).length === 0)
                return toIssue(raw);
            const data = await linearRequest(`mutation($id: String!, $input: IssueUpdateInput!) {
        ...
        async touchClaim(ref, claimant) {
            const raw = await fetchRaw(ref);
            const data = await linearRequest(`query($id: String!, $prefix: String!) {
           issue(id: $id) { ${CLAIM_COMMENT_FIELDS} }
            ...
            await linearRequest(`mutation($id: String!, $input: CommentUpdateInput!) { commentUpdate(id: $id, input: $input) { success } }`, { id: claim.commentId, input: { body: withHeartbeat(body, new Date().toISOString()) } });
            return true;
        },
        ...
        async listClaims() {
            ...
            for (const node of await allNodes(await page(null), page)) {
                ...
                if (claim)
                    claims.push({ issue: toIssue(node), ...claim });
            }
            return claims;
        },
    };
