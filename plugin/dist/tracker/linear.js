...
import { byPriorityThenAge, extractAcceptanceCriteria, LINEAR_TEAM_KEY, } from "./types.js";
const ISSUE_FIELDS = `
  id
  identifier
  title
...
export function createLinearTracker() {
    ...
    async function userIdFor(claimant) {
        const data = await linearRequest(`query($q: String!) {
    ...
    return {
        ...
        async claimIssue(ref, claimant) {
            ...
            if (Object.keys(input).length > 0) {
                await linearRequest(`mutation($id: String!, $input: IssueUpdateInput!) {
