...
import { BOARDS, TASK_COLUMNS, TASK_STATUS } from "../constants.js";
import { getTaskDescriptionDoc } from "../tools/taskDescriptionDoc.js";
import { byPriorityThenAge, extractAcceptanceCriteria, } from "./types.js";
 ...
 */
export function stripDescriptionDocHeader(raw) {
    if (!raw)
        return "";
    const trimmedRaw = raw.trim();
    if (/^(ERROR|Failed)\b/i.test(trimmedRaw))
        return "";
    const lines = raw.split("\n");
    const firstLine = lines[0].trim();
    if (ERROR_HEADING_RE.test(firstLine))
        return "";
...
async function fetchItem(itemId) {
    const data = await executeMondayQuery(`query($ids: [ID!]) {
       items(ids: $ids) {
    ...
    if (!item)
        throw new Error(`Monday: no item ${itemId}`);
    return item;
}
async function toIssue(item) {
...
export function createMondayTracker() {
    return {
        ...
        async claimIssue(ref, claimant) {
            await executeMondayQuery(`mutation($board: ID!, $item: ID!, $column: String!, $value: JSON!) {
           change_column_value(board_id: $board, item_id: $item, column_id: $column, value: $value) { id }
            ...
            await executeMondayQuery(`mutation($item: ID!, $body: String!) {
           create_update(item_id: $item, body: $body) { id }
         }`, { item: ref, body: `claimed by ${claimant} at ${new Date().toISOString()}` });
            return toIssue(await fetchItem(ref));
        },
        async createIssue(input) {
            const data = await executeMondayQuery(`mutation($board: ID!, $name: String!) {
        ...
        async listReady(limit = 25) {
            const data = await executeMondayQuery(`query($board: ID!, $limit: Int!, $column: String!) {
           boards(ids: [$board]) {
             items_page(
               limit: $limit,
