...
echo "--- gate (f): config the push runs under (STEP-3364) ---"
check 2 "cannot tell which branch" "git -c alias.p=push p origin main" "$(payload 'git -c alias.p=push p origin main')"
check 2 "Destructive command detected: 'git config alias" "git config alias.p push && git p origin main" "$(payload 'git config alias.p push && git p origin main')"
check 2 "Destructive command detected: 'git config push" "git config push.default matching && git push" "$(payload 'git config push.default matching && git push')"
check 2 "every branch" "GIT_CONFIG_COUNT=1 GIT_CONFIG_KEY_0=push.default … git push" "$(payload 'GIT_CONFIG_COUNT=1 GIT_CONFIG_KEY_0=push.default GIT_CONFIG_VALUE_0=matching git push')"
check 2 "every branch" "export GIT_CONFIG_…; git push" "$(payload 'export GIT_CONFIG_COUNT=1 GIT_CONFIG_KEY_0=push.default GIT_CONFIG_VALUE_0=matching; git push')"
check 2 "cannot tell which branch" "GIT_DIR=<a checkout on main> git push origin HEAD" "$(payload "GIT_DIR=$MAIN_WT/.git git push origin HEAD")"
check 2 "protected branch 'main'" "nice -n 5 git push origin main" "$(payload 'nice -n 5 git push origin main')"
check 2 "protected branch 'main'" "env -S 'git push origin main'" "$(payload "env -S 'git push origin main'")"
check 2 "protected branch 'main'" "timeout 30 git push origin main" "$(payload 'timeout 30 git push origin main')"
check 2 "Destructive command detected: 'git push --force'" "timeout 30 git push --force origin feat/x" "$(payload 'timeout 30 git push --force origin feat/x')"
check 2 "protected branch 'main'" "/usr/bin/env git push origin main" "$(payload '/usr/bin/env git push origin main')"
check 2 "protected branch 'main'" "script -q /dev/null git push origin main" "$(payload 'script -q /dev/null git push origin main')"
check 2 "cannot tell which branch" "flock /tmp/l git push origin main: a wrapper it does not know" "$(payload 'flock /tmp/l git push origin main')"
check 2 "protected branch 'main'" "env -S 'git\\_push\\_origin\\_main'" "$(payload "env -S 'git\\_push\\_origin\\_main'")"
