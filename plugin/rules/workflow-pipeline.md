...
backlog → Ready to Start → In Progress → Waiting for UAT → Pending Deploy to Prod → Done
   ...
   (worktree +            /self-review +    on test env        sets Done +
   active-task.json)      /ship-pr (PR +    → flips status     generates changelog
                          merge)
            │                  │                │                   │
   refinement-gate    bash-guard         demo-url-required   pre-merge-review-gate
   (claim gate)       (commit + push     (Waiting for UAT     (gh pr merge gate)
                      gates)              gate)
                      protect-active-     stop-waiting-for-
                      task-state          uat-stage (Stop)
                      (state file)        stop-monday-
                                          reconciled-check
                                          (Stop)
```
...
| **Review loop** | (stays `In Progress`) | Agent (main session w/ `Monitor`): polls `gh pr checks {N}` + Corridor + bot comments, triages, fixes BLOCKERs, re-pushes. Repeat until green + reviews addressed. Writes structured `reviewAddressed` to state file. Subagent path: sets `reviewAddressed: "handoff-to-orchestrator"` and SendMessage's the orchestrator, who runs `/dev-tasks:babysit-prs`. | `pre-merge-review-gate` (PreToolUse `gh pr merge`) — refuses merge until `reviewAddressed` is set with a triage timestamp newer than the latest bot review. | [`agent-orchestration.md`](agent-orchestration.md) for subagent → orchestrator handoff |
| **Merge** | (stays `In Progress`) | Agent: `gh pr merge --admin --squash` (never `--delete-branch` — collides with worktrees). Waits for the post-merge deploy to be `READY` before claiming "verified" (PR #347 / retro #2926719311 case study). | `post-merge-postmortem.sh` (PostToolUse) captures the merge event. | `CLAUDE.md` "Shipping conventions" → "Deploy-lag gotcha" |
| **WfUAT transition** | `In Progress` → `Waiting for UAT` | Default flow: `/ship-pr` Phase 6.5 calls `updateTask({status: "Waiting for UAT"})` after subtasks all Done + UAT doc set. Auto-pulled into active sprint if missing. Mirror to state file: `parentStatus: "Waiting for UAT"`. | `demo-url-required` (PreToolUse `updateTask`) — refuses without `demoUrl` matching `project-config.ci.previewUrlPattern`. `stop-waiting-for-uat-stage` (Stop) — refuses session exit when subtasks Done but parent not flipped. | [`task-lifecycle.md`](task-lifecycle.md) status gates |
...
| **Release** | `Pending Deploy to Prod` → `Done` | Human or agent: `/dev-tasks:release-version`. Creates/updates version, links tasks, generates changelog doc, flips tasks → Done. | None blocking (server-side validation in `generateChangelog`). | [`release-flow.md`](release-flow.md), [`versions-lifecycle.md`](versions-lifecycle.md), [`versioning.md`](versioning.md) |
...
- `stop-waiting-for-uat-stage` + `stop-monday-reconciled-check` depend on
  `/ship-pr` Phases 6.5 + 10 writing `parentStatus` + `mondayReconciledShas[]`.
