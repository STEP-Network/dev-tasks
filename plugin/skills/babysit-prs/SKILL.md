...
4. If parent status not `Waiting for UAT`:
   ...
   - **Full-suite E2E advisory run (v0.34.0)** — once the staging deploy is `READY` (above), invoke `/dev-tasks:run-full-e2e --taskId=<taskId>` to run the consumer's full Playwright suite in-session against staging and record the result (this is the orchestrator-side analog of `/ship-pr` step 20f.8, including any after pass `visualDiff.afterPending` deferred to this loop). ADVISORY — it never gates; the skill's own safe-skip makes it a recorded no-op for projects without a staging URL + a real suite, and it never errors. Fold the `fullSuiteE2E` result into the per-PR summary below.
