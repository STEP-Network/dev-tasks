...
1. **Check for duplicates**: Use `mcp__dev-tasks__getBacklog` to search for similar/related tasks.
...
2. **Epic assignment** (MANDATORY — no orphaned tasks):
   - Call `mcp__dev-tasks__listEpics(product: "PolAds")` to get available epics.
...
7. **Create task**: call `mcp__dev-tasks__createTask`. Recommended fields:
