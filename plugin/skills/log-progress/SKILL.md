...
3. **Post structured update** to Monday.com via `mcp__dev-tasks__createUpdate`
4. **Manage subtask lifecycle** (if SUBTASK_COMPLETED):
   ...
   b. Fetch subtask's `started_date` from Monday.com via `mcp__dev-tasks__getTask` (canonical source)
   c. Calculate `actualHours = (now - started_date)` in hours, rounded to 1 decimal
   d. Update Monday.com subtask: set status "Done" + set actual hours via `mcp__dev-tasks__manageSubtasks`
   e. Update state file: set subtask `"status": "done"`, add `"completedAt"` and `"actualHours"`
   f. Set NEXT subtask to "In Progress" on Monday.com via `mcp__dev-tasks__manageSubtasks` (triggers `started_date`)
   g. Update state file: next subtask becomes `"status": "in_progress"` with `"mondayStartedDate"` from Monday.com
   h. Post structured update via `mcp__dev-tasks__createUpdate` (this replaces step 3 for SUBTASK_COMPLETED — do NOT double-post)
...
3. Include PR link, preview URL, and current task status (from `mcp__dev-tasks__getTask`) if available.
4. Post summary via `mcp__dev-tasks__createUpdate` with full estimate/actual breakdown.
