...
6.8. **VisualDiff BEFORE pass (v0.33.0; deterministic gate + audited skip v0.34.0)** — captures the changed UI on staging *before* this change ships, so the human gets a before/after pair in a Monday doc. Runs HERE (pre-push) because staging still shows the pre-change state until the PR merges + deploys.
   ...
   - **MANDATORY record (v0.34.0)** — when the gate says UI, you MUST end this step having written EITHER `visualDiff.routes` (the captured route list, below) OR `visualDiff.skipReason` (a one-line, honest reason) into `.claude/active-task.json`. Mirror the skip reason into the PR body. `stop-visual-diff-check` reads exactly these two fields and BLOCKS session exit on a UI diff with neither set — silent self-skip is no longer possible.
     - **"No local build" is NOT a valid skip reason** when an `https://` staging URL is configured: staging IS screenshot-reachable without a local build — `npx playwright screenshot` against the staging URL (or chrome-devtools-mcp with file save, for public routes). Valid skips are e.g. "no resolvable staging URL for this route", "route requires auth and no persona configured", "diff is non-UI".
   ...
   - **Auth (per-route persona resolution — v0.37.0)** — public routes need none. For an authed route, resolve its persona: the matching `routeMap` entry's `persona` (when the route came from a routeMap match) → `visualDiff.authPersona` → none. A resolved persona authenticates the **Playwright** capture below via its `storageState` (`e2e.personas[]`) — MCP browsers cannot consume a Playwright storageState. For an MCP-browser capture the ONLY sanctioned login is `visualDiff.loginUrl` (a consumer-declared programmatic test-login route): substitute `{persona}` with the resolved persona id and `{secret}` with the value of the env var named by `visualDiff.loginSecretEnv` (resolve AT NAVIGATION TIME; never echo the substituted URL into logs, the PR, or the Monday doc), assert its origin EQUALS the `environments.uat.url` origin (same SSRF guard as above), navigate to it once, then capture — with a FILE-SAVING tool per the Capture contract below (`chrome-devtools-mcp` writes a file and so CAN deliver an uploadable authed capture after a loginUrl login; `claude-in-chrome` remains note-only even after login). No resolvable persona/storageState AND no usable loginUrl → SKIP that route with a noted reason (`note: "skipped — auth required, no persona"`) — never guess a login (same contract as `write-uat-spec`).
   - **Capture (file-producing tools ONLY for uploads — v0.37.0 HARD RULE)** — `appendTaskVisualSnapshots` embeds by absolute local `imagePath`, so an uploaded capture REQUIRES a tool that writes the PNG to DISK. Per route × viewport (`visualDiff.viewports`, default desktop 1440×900 + mobile 390×844, full page):
     1. **Playwright CLI is the DEFAULT for all uploads — and REQUIRED for storageState-authenticated routes** (it is the only path that consumes storageState): `npx playwright screenshot --viewport-size=1440,900 --full-page --wait-for-timeout=3000 [--load-storage=<persona storageState path>] "<staging-url>" "<outdir>/<slug>-desktop.png"` (repeat with `390,844` for mobile).
     2. `chrome-devtools-mcp` `take_screenshot` MAY capture public routes — or authed routes reached via a `visualDiff.loginUrl` login (Auth bullet above) — when invoked with its file-saving parameter. Verify the file exists on disk before listing its path as an `imagePath`.
     3. `claude-in-chrome` screenshots land in model context, NOT on disk — they can NEVER feed an upload. If a page was only eyeballed there, its doc entry is note-only: `note: "verified visually in browser — no file-producing capture"`.
     A pass that "captured" without files on disk is NOT a capture — record note-only entries with the explicit reason; a silent image-less pass is the exact failure mode this rule kills (a before/after doc of prose with zero screenshots). Save PNGs under a temp dir (e.g. `mktemp -d` or `.claude/visual-snapshots/<taskId>/before/`). A route that 404s on staging (new route) → record a note-only capture (`note: "no before state — new route"`, no imagePath).
...
30. Post-merge sequence (order matters for `allowMainCheckout: true` — `gh pr merge` switches to `$defaultBase` and deletes local branch):
    ...
    - `$defaultBase`: leave task at `Waiting for UAT`. No `[TASK_COMPLETED]` narrative post — the status (already at `Waiting for UAT` from Phase 6.7) carries the next-transition signal (Waiting for UAT → Pending Deploy to Prod by human; Pending Deploy to Prod → Done by the opt-in `complete-released-tasks` sweep, `plugin/templates/github-workflows/` — `/release-version` itself deliberately never sets task status, see `release-version/SKILL.md` Step 6).
