...
13. **Post preview URL + link metadata to Monday.com**: Call `mcp__plugin_dev-tasks_dev-tasks__updateTask` with:
...
14d. **Create or update the UAT doc**:
    - First call attempt: `mcp__plugin_dev-tasks_dev-tasks__createTaskUatDoc({ taskId, markdown })`.
    - If response indicates a doc already exists (re-push scenario): call `mcp__plugin_dev-tasks_dev-tasks__updateTaskUatDoc({ taskId, markdown, overwrite: true })` instead.
...
20b. **Verify gate prereqs** (the MCP enforces these server-side; this is early validation for clearer errors):
    ...
    - UAT doc exists on column `doc_mm3adfdg` — verify via `mcp__plugin_dev-tasks_dev-tasks__getTaskUatDoc(taskId)`. If absent, re-run Phase 4.5.
...
20c. **Transition status**:
    ```
    mcp__plugin_dev-tasks_dev-tasks__updateTask({
      itemId: taskId,
...
22. **Refresh Vercel preview URL**: Use `mcp__vercel__list_deployments` to get the latest
    ...
    and `demoUrl` on Monday.com via `mcp__plugin_dev-tasks_dev-tasks__updateTask` if it changed.

23. **Update Monday.com parent task**: Use `mcp__plugin_dev-tasks_dev-tasks__createUpdate` to post:
    ```
...
25b. **Update structured Release Summary** (after version is confirmed linked):
    ...
    - Call `mcp__plugin_dev-tasks_dev-tasks__getVersion(versionId)` to get current Release Summary and task details
    ...
    **3-category mapping (Feature / Improvement / Fix)** — established 2026-05-07:
    ...
    - **If structured format exists** (has `STRUCTURED_CHANGELOG_V1` markers):
      ...
      d. Write back: `mcp__plugin_dev-tasks_dev-tasks__updateVersion(versionId, releaseSummary: updatedJSON)`.
    - **If plain text or empty** (first task shipped for this version):
      ...
      e. Write back: `mcp__plugin_dev-tasks_dev-tasks__updateVersion(versionId, releaseSummary: structuredContent)`.
...
28. **Post checklist to Monday.com**: Use `mcp__plugin_dev-tasks_dev-tasks__createUpdate` to post the same
...
30. **After `gh pr merge` succeeds** (or when the agent detects the PR state is "MERGED"):
    ...
    - **Hotfix flow (`baseRefName == "main"`)**:
      a. Call `mcp__plugin_dev-tasks_dev-tasks__updateTask(itemId, status: "Done")`. The MCP gate accepts this because all subtasks are `Done` (hotfix had no UAT doc requirement).
      b. Post final update:
        ```
        [TASK_COMPLETED] Agent Progress Update
        Time: {ISO 8601} | Branch: {branch}
