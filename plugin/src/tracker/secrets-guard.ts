...
const SECRET_RE =
  /lin_api_[A-Za-z0-9]{8,}|xox[abpr]-[A-Za-z0-9-]{6,}|xapp-[A-Za-z0-9-]{6,}|sk-ant-[A-Za-z0-9_-]{8,}|gh[pousr]_[A-Za-z0-9]{20,}|github_pat_[A-Za-z0-9_]{20,}|\b(LINEAR_API_KEY|SLACK_BOT_TOKEN|SLACK_APP_TOKEN|CLAUDE_CODE_OAUTH_TOKEN|SENTRY_CRON_URL)\s*=/

/** Throws when `text` looks like it carries a key or a token. `what` names the flag, never the text. */
export function assertNoSecretText(text: string, what: string): void {
 ...
 */
export function readTextFile(path: string, what: string, home: string = homedir()): string {
  const refuse = () => {
